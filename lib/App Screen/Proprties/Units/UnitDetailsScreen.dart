...
class _UnitDetailsScreenState extends State<UnitDetailsScreen> {
  ...
  int _currentImageIndex = 0;
  List<String> imageBase64List = []; // To store Base64 images

  @override
  void initState() {
    super.initState();
    _fetchUnitImages(); // Fetch images when the screen is initialized
  }

  Future<void> _fetchUnitImages() async {
    try {
      final DataSnapshot snapshot =
          await _realtimeDb.child('unitImages/${widget.unit.documentId}').get();

      if (snapshot.value != null) {
        Map<dynamic, dynamic> images = snapshot.value as Map<dynamic, dynamic>;

        // Extract all Base64 strings into a list
        setState(() {
          imageBase64List = images.values
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                "widget.unit.description ?? No description provided.",
                style: const TextStyle(
                  fontSize: 13.8,
                  fontWeight: FontWeight.w400,
                  color: Color.fromRGBO(115, 115, 115, 1),
