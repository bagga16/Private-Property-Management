...
class NumberOfUnitsFilter extends StatefulWidget {
  final Function(int? minUnits, int? maxUnits) onApply;

  const NumberOfUnitsFilter({super.key, required this.onApply});
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    if (_selectedUnits == "Manually") {
                      minUnits = int.tryParse(_fromController.text);
                      maxUnits = int.tryParse(_toController.text);
                    } else if (_selectedUnits == "Above 20") {
                      minUnits = 21;
                    } else if (_selectedUnits == "Below 20") {
                      maxUnits = 19;
                    } else {
                      minUnits = maxUnits = int.tryParse(_selectedUnits!);
                    }
                    widget.onApply(minUnits, maxUnits);
                    Navigator.pop(context);
                  },
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color.fromRGBO(139, 200, 63, 1),
